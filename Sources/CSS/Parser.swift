...
public indirect enum ComponentValue: CustomStringConvertible {
    ...
    public var description: String {
        return switch self {
            case .token(let token): "CV(\(token))"
            case .function(let f): "CV(\(f))"
            case .simpleBlock(let simpleBlock): "CV(\(simpleBlock))"
        }
    }
    
    public func ident() -> String? {
        if case .token(.ident(let ident)) = self {
            return ident
        }
        return nil
    }
...
public enum Rule: CustomStringConvertible {
    ...
    public var description: String {
        return switch self {
            case .at(let at): "R: \(at)"
            case .qualified(let qualified): "R: \(qualified)"
        }
    }
...
public struct ParsedStyleSheet: CustomStringConvertible {
    ...
    public func qualifiedRuleTokens() -> [Token] {
        var tokens: [Token] = []
        for case .qualified(let qualifiedRule) in self.rules {
...
func consumeAtRule(_ tokens: inout TokenStream, name: String) throws -> AtRule {
    ...
    repeat {
        switch try tokens.consumeNextInputToken() {
            ...
            case .componentValue(.simpleBlock(let simpleBlock)):
...
func consumeQualifiedRule(_ parser: inout TokenStream) throws -> Optional<QualifiedRule> {
...
func consumeStyleBlock(_ tokenStream: inout TokenStream) throws -> [Item] {
    ...
    repeat {
        switch try tokenStream.consumeNextInputToken() {
            ...
            case .token(.delim(let value)) where value == "&":
...
func consumeDeclaration(_ tokenStream: inout TokenStream) throws -> Optional<Declaration> {
...
func consumeComponentValue(_ tokenStream: inout TokenStream) throws -> ComponentValue {
    ...
    switch try tokenStream.consumeNextInputToken() {
        ...
        case .token(.lcurlybracket), .token(.lbracket), .token(.lparan):
            ...
            return ComponentValue.simpleBlock(try consumeSimpleBlock(&tokenStream))
        
        // Otherwise, if the current input token is a <function-token>
        case .token(.function(let name)):
            // consume a function and return it.
            return ComponentValue.function(try consumeFunction(&tokenStream, name: name))
        
        // Otherwise
        case let inputToken:
            // return the current input token.
            return switch inputToken {
                case .componentValue(let componentValue): componentValue
                case .token(let token): ComponentValue.token(token)
            }
...
func consumeSimpleBlock(_ tokenStream: inout TokenStream) throws -> SimpleBlock {
    ...
    repeat {
        switch try tokenStream.consumeNextInputToken() {
            ...
            // ending token
            case .token(let token) where token == endingToken:
                // Return the block.
