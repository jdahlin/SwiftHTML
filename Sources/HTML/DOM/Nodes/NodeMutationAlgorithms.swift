extension DOM {
    ...
    static func ensurePreInsertValidation(node: DOM.Node, parent: DOM.Node, child: DOM.Node?) throws {
        // If parent is not a Document, DocumentFragment, or DOM.Element node, then throw a
        // "HierarchyRequestError" DOMException.
        guard parent is DOM.Document || parent is DOM.DocumentFragment || parent is DOM.Element else {
            throw DOMException.hierarchyRequestError
        }

        // If node is a host-including inclusive ancestor of parent, then throw a
        // "HierarchyRequestError" DOMException.
        guard !DOM.isHostIncludingInclusiveAncestor(a: node, b: parent) else {
            throw DOMException.hierarchyRequestError
        }
        ...
        // If node is not a DocumentFragment, DocumentType, DOM.Element, or CharacterData
        // node, then throw a "HierarchyRequestError" DOMException.
        guard node is DOM.DocumentFragment || node is DOM.DocumentType || node is DOM.Element || node is DOM.CharacterData
        else {
            throw DOMException.hierarchyRequestError
        }
        ...
        // DOMException.
        if (node is DOM.Text && parent is DOM.Document) || (node is DOM.DocumentType && !(parent is DOM.Document)) {
            throw DOMException.hierarchyRequestError
        }
    ...
    // is following child.
            ...
            case is DOM.Element:
                ...
                if Array(parent.childNodes).allSatisfy({ $0 is DOM.Element }), child is DOM.DocumentType /* || (child != nil && parent.hasFollowingDoctype(child!) )*/
                {
                    throw DOMException.hierarchyRequestError
                }
            // DocumentType
    ...
    @discardableResult static func preInsertBeforeChild(node: DOM.Node, parent: DOM.Node, child: DOM.Node?) -> DOM.Node {
    ...
    static func insertNodeIntoParent(node: DOM.Node,
                                     parent: DOM.Node,
                                     child: DOM.Node?,
                                     suppressObservers _: Bool)
    {
        // 1. Let nodes be node’s children, if node is a DocumentFragment node; otherwise « node ».
        let nodes = if let fragment = node as? DOM.DocumentFragment {
        ...
        if let _ = node as? DOM.DocumentFragment {
    ...
    static func appendNodeToParent(node: DOM.Node, parent: DOM.Node) -> DOM.Node {
        // To append a node to a parent, pre-insert node into parent before null.
        preInsertBeforeChild(node: node, parent: parent, child: nil)
    }

    // https://dom.spec.whatwg.org/#concept-node-replace
    static func replaceChild(child: DOM.Node, node: DOM.Node, parent _: DOM.Node) -> DOM.Node {
        // 1. If parent is not a Document, DocumentFragment, or DOM.Element node, then
        ...
        // 4. If node is not a DocumentFragment, DocumentType, DOM.Element, or
        ...
        case is DOM.Element:
            ...
            // following child.
            DIE("DOM.Element not implemented")
        // DocumentType
        case is DOM.DocumentType:
    ...
    static func replaceAll(node: DOM.Node?, parent: DOM.Node) {
        // 1. Let removedNodes be parent’s children.
        let removedNodes = parent.childNodes

        // 2. Let addedNodes be the empty set.
        var addedNodes = Set<DOM.Node>()

        // 3. If node is a DocumentFragment node, then set addedNodes to node’s children.
        if let fragment = node as? DOM.DocumentFragment {
    ...
    static func removeNode(node: DOM.Node, suppressObservers _: Bool = false) {
    ...
    static func isChildAllowed(node: DOM.Node) -> Bool {
        switch node {
        case is DOM.Document: false
        case is DOM.Text: true
        case is DOM.Comment: true
        case is DOM.DocumentType: !(node.firstChild is DOM.DocumentType)
        case is DOM.Element: !(node.firstChild is DOM.Element)
        default: false
        }
    }

    static func appendChildImpl(parent: DOM.Node, node: DOM.Node) {
    ...
    static func insertBeforeImpl(parent: DOM.Node, node: DOM.Node, child childOrNone: DOM.Node?) {
        if childOrNone == nil {
            appendChildImpl(parent: parent, node: node)
            return
        }
    ...
    static func removeChildImpl(parent: DOM.Node, node: DOM.Node) {
